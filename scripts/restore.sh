...
get_resurrect_file() {
    if [ -n "$CUSTOM_RESTORE_FILE" ]; then
        echo "$(resurrect_dir)/$CUSTOM_RESTORE_FILE"
    else
        echo "$(last_resurrect_file)"
    fi
}

check_saved_session_exists() {
    local resurrect_file="$(get_resurrect_file)"
...
restore_window_properties() {
    local window_name
    \grep '^window' "$(get_resurrect_file)" |
...
restore_all_pane_processes() {
    if restore_pane_processes_enabled; then
        ...
        awk 'BEGIN { FS="\t"; OFS="\t" } /^pane/ && $11 !~ "^:$" { print $2, $3, $6, $8, $11; }' "$(get_resurrect_file)" |
...
restore_active_pane_for_each_window() {
    awk 'BEGIN { FS="\t"; OFS="\t" } /^pane/ && $9 == 1 { print $2, $3, $6; }' "$(get_resurrect_file)" |
...
restore_zoomed_windows() {
    awk 'BEGIN { FS="\t"; OFS="\t" } /^pane/ && $5 ~ /Z/ && $9 == 1 { print $2, $3; }' "$(get_resurrect_file)" |
...
restore_active_and_alternate_windows() {
    awk 'BEGIN { FS="\t"; OFS="\t" } /^window/ && $6 ~ /[*-]/ { print $2, $5, $3; }' "$(get_resurrect_file)" |
        sort -u |
